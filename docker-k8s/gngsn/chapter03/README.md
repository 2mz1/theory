...
애플리케이션과 컨테이너가 전체 시스템에서 차지해야 하는 적정 비중을 고려하면,
"각 컨테이너가 맡은 역할을 적절히 나누고, 그 역할에 따라 배치한 컨테이너를 복제해도 전체 구조에서 부작용이 일어나지 않는가?" 
를 따져가며 시스템을 설계해야 함
...
- 도커가 설치된 환경이라면 어떤 호스트 운영체제와 플랫폼, 온프레미스 및 클라우드 환경에서도 그대로 동작
...
<small>가령, 라즈베리 파이(Raspberry Pi, ARM 계열의 armv71 아키텍처)와 인텔 x86_64 아키텍처에서 빌드한 도커 컨테이너 호환 불가</small>
...
- **정적 링크**
  ...
  - 애플리케이션의 크기가 비대해지는 경향의 있지만 이식성은 뛰어남
- **동적 링크**
  - 애플리케이션이 실행할 때 라이브러리가 링크
  - 애플리케이션 크기는 작아지지만 애플리케이션을 실행할 호스트 라이브러리를 갖춰야 함
...
- 호스트와 컨테이너 사이의 디렉터리 공유 및 재사용 기능을 제공
  - 도커 이미지를 새로 생성해거나 수정해도 동일한 데이터 볼륨 사용
  - 도커 컨테이너 안의 디렉터리를 디스크에 Persistence 데이터로 남기기 위한 메커니즘
- 상태를 갖는 애플리케이션을 실행하는데 적합
- 컨테이너 내 설정 파일을 쉽게 수정할 수 있지만, 특정 호스트 파일 경로에 의존성 발생

<br/>

#### 데이터 볼륨 생성: `docker run -v` option
...
<br/><br/><img src="./image/image01.png" width="70%" /><br/><br/>

**데이터 볼륨 컨테이너 & 호스트 머신**

- "데이터 볼륨"과 공통점: 데이터 볼륨 컨테이너가 공유한 디렉터리 역시 호스트 머신의 스토리지에 저장
...
### 데이터 볼륨 컨테이너에 MySQL 데이터 저장

**예제**: 볼륨을 붙인 MySQL 컨테이너에 데이터를 넣고 컨테이너를 종료 -> 재실행해도 데이터가 남아있는지 확인

<table>
<tr>
<td width="40%">Data Volume Container 생성</td>
<td width="60%">
...
<tr>
<td>Data Volume Container 실행</td>
<td>
...
<tr>
<td>컨테이너 정지 후 동일한 볼륨 컨테이너의의 새로운 컨테이너 생성</td>
<td>
...
<table>
<tr><th colspan="2">✔️ 구현 실습</th></tr>
<tr><td colspan="2">

<br/><img src="./image/image02.png" /><br/>
...
<tr>
<td width="40%">✔️ 1. docker-compose 실행</td><td width="60%">

`/swarm` directory 생성 후, swarm/docker-compose.yml 실행
...
<tr>
<td>✔️ 2. `docker swarm init` 명령을 실행해서 manager 역할 할당</td><td>

`swarm init` 명령어로 도커를 manager로 지정

```
❯ docker container exec -it manager docker swarm init
Swarm initialized: current node (fawlx5ocpkl9rmnesxiacr2c4) is now a manager.

To add a worker to this swarm, run the following command:

    docker swarm join --token SWMTKN-1-41nd7ypcxazy5a2ibkox8xvm2ghd3mn8ahrbzcwizxdhpczjf1-7d3zjzcdxpk7co3urqt6pkpil 172.21.0.3:2377
...
```

- `swarm init` 명령 성공
  - 해당 도커 호스트는 manager로 마킹 → Swarm 모드 활성화
  - join 토큰 생성 및 출력됨

</td></tr>
<tr><td>✔️ 3. docker container 등록해서 클러스터 형성</td><td>

worker 컨테이너에서 manager 컨테이너를 식별하도록 manager:2377 로 join 토큰 전달

```
❯ docker container exec -it worker01 docker swarm join --token SWMTKN-1-41nd7ypcxazy5a2ibkox8xvm2ghd3mn8ahrbzcwizxdhpczjf1-7d3zjzcdxpk7co3urqt6pkpil manager:2377
This node joined a swarm as a worker.
❯ docker container exec -it worker02 docker swarm join --token SWMTKN-1-41nd7ypcxazy5a2ibkox8xvm2ghd3mn8ahrbzcwizxdhpczjf1-7d3zjzcdxpk7co3urqt6pkpil manager:2377
...
❯ docker container exec -it worker03 docker swarm join --token SWMTKN-1-41nd7ypcxazy5a2ibkox8xvm2ghd3mn8ahrbzcwizxdhpczjf1-7d3zjzcdxpk7co3urqt6pkpil manager:2377
...
```

</td></tr>
<tr><td>✔️ 4. Swarm cluster 상태 확인</td><td>

```
❯ docker container exec -it manager docker node ls
ID                            HOSTNAME       STATUS    AVAILABILITY   MANAGER STATUS   ENGINE VERSION
26g1xgisab3bl6zo0exxfbvwq     48d7874769d7   Ready     Active                          24.0.6
kqrmlwx5y46wamtsd2rgdfbnl     01695eeab406   Ready     Active                          24.0.6
xt6tonlv2py0bwzjhr1o3itk5     a561e3b76ddf   Ready     Active                          24.0.6
fawlx5ocpkl9rmnesxiacr2c4 *   c9fae265ed96   Ready     Active         Leader           24.0.6
```
...
<table>
<tr>
<td width="40%">✔️ 1. docker image tag</td>
<td width="60%">
...
<tr>
<td>✔️ 2. registry 컨테이너에 이미지 등록</td>
<td>
...
<tr>
<td>✔️ 3. registry 컨테이너에서 이미지 내려받기</td>
<td>
...
cbcf842c718c: Download complete
...  
Status: Downloaded newer image for registry:5000/gngsn/echo:latest
registry:5000/gngsn/echo:latest
```
...
<tr>
<td>✔️ 4. 내려받은 docker image 확인</td>
<td>

```bash
❯ docker container exec -it worker01 docker image ls
REPOSITORY                 TAG       IMAGE ID       CREATED         SIZE
registry:5000/gngsn/echo   latest    7e6397336ef5   6 minutes ago   662MB
```
...
<table>
<tr>
<td width="40%">✔️ 1. 서비스 생성</td>
<td width="60%">

```bash
❯ docker container exec -it manager docker service create --replicas 1 --publish 8000:8000 --name echo registry:5000/gngsn/echo:latest
...
6royaikec2pg   echo      replicated   1/1        registry:5000/gngsn/echo:latest   *:8000->8000/tcp
```
...
<tr>
<td width="40%">✔️ 3. 배포된 스택 배포 확인</td>
<td width="60%">
...
<tr>
<td width="40%">✔️ 4. 스택에 배포된 컨테이너 확인하기</td>
<td width="60%">
...
<tr>
<td width="40%">✔️ + visualizer를 사용해 컨테이너 배치 시각화</td>
<td width="60%">
...
```bash
❯ docker container exec -it manager docker stack deploy -c ./stack/ch03-webapi.yml ingress
```
