...
- 복잡도 또는 도메인 유의성과 협력자 수에 따른 네 가지 유형의 코드
  1. 도메인 모델 및 알고리즘(복잡도 또는 도메인 유의성이 높음, 협력자가 거의 없음)은 단위 테스트에 대한 노력 대비 가장 이로움
  2. 간단한 코드(복잡도와 도메인 유의성이 낮음. 협력자가 거의 없음)는 테스트할 가치가 전혀 없다.
  3. 컨트롤러(복잡도와 도메인 유의성이 낮음. 협력자가 많음)는 통합 테스트를 통해 간단 히 테스트해야 한다.
  4. 지나치게 복잡한 코드(복잡도 또는 도메인 유의성이 높음. 협력자가 많음)는 컨트롤리 와 복잡한 코드로 분할해야 한다.
...
- **코드의 깊이와 너비**의 관점에서 비즈니스 로직과 오케스트레이션 책임을 생각하라
...
- 의사 결정 프로세스 단계를 더 세분화하는 것이 장단점을 고려할 때 가장 효과적인 절충 
